...
sessiontrail is a Claude Code plugin that records what actually happened in a project (sessions, decisions, completed work, and the files they touched) and renders it as an interactive left-to-right timeline.
...
sessiontrail captures history three ways, layered so you get a useful timeline even if you never think about it:

1. **Automatic (hooks)**: every session registers a lane, every file you create or edit is captured as an artifact touch, and the session title is backfilled automatically. Zero effort, zero latency (recording hooks run async).
...
```
/plugin marketplace add jsk4581/sessiontrail
/plugin install sessiontrail@sessiontrail
```
...
|---|---|
| `/sessiontrail:view` | Start the timeline viewer and get its URL |
| `/sessiontrail:note <what happened>` | Manually record a milestone |
...
```bash
node bin/sessiontrail.mjs add       # append a milestone (JSON on stdin)
node bin/sessiontrail.mjs recent    # list recent nodes (merge targets)
node bin/sessiontrail.mjs serve     # start the viewer [--host] [--port]
node bin/sessiontrail.mjs build     # force-rebuild the graph cache
node bin/sessiontrail.mjs doctor    # sanity-check the installation
```

## Data & privacy

All data lives in **`.sessiontrail/` inside your project**: a single append-only `events.jsonl` plus a derived graph cache. Nothing leaves your machine.

- **Commit `.sessiontrail/`** to share the timeline with your team, or
