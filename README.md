...
- **Judgment is recorded, not just parsed.** A code graph is built by parsing; a model only summarizes it. Here the agent doing the work decides what mattered, and a Sonnet scribe subagent writes it down in the background, so the conversation is never slowed.
