...
/**
 * @param {object} o
 * @param {string} o.cliPath absolute path to bin/sessiontrail.mjs
...
export function buildProtocol({ cliPath, recent = [] }) {
  ...
  return `sessiontrail is active: this project's development history is recorded as a timeline.
When you make a significant decision or complete a significant piece of work, hand the write-up to the sessiontrail scribe subagent. Do not write the milestone yourself.

Delegate with the Agent tool: subagent_type "sessiontrail:sessiontrail-scribe", model "sonnet", run in the background and keep working (never wait for it). Give it a brief:
