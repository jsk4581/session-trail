...
Ask the timeline only before undoing something that looks deliberate (an existing rule, data format, or architecture choice). Run one of
  node "${cliPath}" why <path> | search <term> | show <id>
(milestones behind a file or directory; milestones and user prompts containing a phrase; a node's what/why/how). Never query for new files, routine edits, or questions. Most turns need no query at all.
